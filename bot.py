...
TOKEN = 'MTAyODM4OTM1OTc1MTI3NDU0Ng.G29LkZ.fb_csxTUl2eeYPlcVEhs9EcPT_LcmfnCPTQdRY'
...
async def craft(context, item, count=1):
    ...
    embed = discord.Embed(
    	title = str(count) + ' ' + item
	)
	
    for i in range(1,4): 
        req = str(df['Req'+str(i)].loc[df['Item'] == item].iloc[0])
        req_count = str(df['Req'+ str(i)+' Units'].loc[df['Item'] == item].iloc[0]*count)
        
        if(req != 'nan'):
            sub_req1 = str(df['Req1'].loc[df['Item'] == req].iloc[0])
            sub_req1_count = str(df['Req1 Units'].loc[df['Item'] == req].iloc[0]*count)
            
            sub_req2 = str(df['Req2'].loc[df['Item'] == req].iloc[0])
            sub_req2_count = str(df['Req2 Units'].loc[df['Item'] == req].iloc[0]*count)
            
            sub_req3 = str(df['Req3'].loc[df['Item'] == req].iloc[0])
            sub_req3_count = str(df['Req3 Units'].loc[df['Item'] == req].iloc[0]*count)
